...
import { GaleriaGridWrpr, GaleriaSection } from '../components/GaleriaSection/Galeria.styled';
...
const Home: NextPage = (props: InferGetStaticPropsType<typeof getStaticProps>) => {
  const imagins: InstaInterface[] = props.instadata.data.filter((img: { media_type: string; }) => img.media_type === 'IMAGE' || img.media_type === 'CAROUSEL_ALBUM').slice(0,9);
  ...
  return (
    <>
        ...
        <RedSection id='horarios'>
          ...
          <HorariosGrid>
            {
              horarios.sort((a,b) => a.order - b.order).map((wrk) => (
                <Fragment key={wrk.id}>
                  <div>
                    <p>{wrk.dia}</p>
                  </div>
        ...
        <GaleriaSection id='galeria'>
          ...
          <GaleriaGridWrpr>
            {
            imagins && imagins.map((imagin) => (
              <div key={imagin.id}>
                <a href={imagin.permalink} target="_blank" rel="noopener noreferrer">
